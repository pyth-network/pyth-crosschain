...
const MAGIC: vector<u8> = b"PTGM";

/// Governace message module. Always 3, as this contract uses "Lazer" actions.
const MODULE: u8 = 3;

#[error]
const EMismatchedMagic: vector<u8> = b"Mismatched governance header magic number, should be \"PTGM\"";
#[error]
const EMismatchedModule: vector<u8> = b"Mismatched governance header module number, should be 3";
#[error]
const EMismatchedEmitterChainID: vector<u8> = b"Mismatched governance emitter chain ID";
#[error]
const EMismatchedReceiverChainID: vector<u8> = b"Mismatched governance receiver chain ID";
#[error]
const EMismatchedAddress: vector<u8> = b"Mismatched governance emitter address";
#[error]
const EOldSequenceNumber: vector<u8> = b"Incoming sequence number older than previously seen";
...
// predicates to allow modification in the future, as Iota types cannot be
// private (yet?):
