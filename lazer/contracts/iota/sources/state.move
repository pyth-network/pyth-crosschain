...
#[error]
const EInvalidPubkeyLen: vector<u8> = b"Invalid public key length, must be 33";
#[error]
const ERemovedSignerNotFound: vector<u8> = b"Could not remove non-existent trusted signer";
#[error]
const EDifferentVersion: vector<u8> = b"State can only be used with the current package version";
#[error]
const EDifferentUpgradeCap: vector<u8> = b"Supplied UpgradeCap belongs to a different package";
...
///
/// Always use the latest version of the Iota Lazer contract, as older versions
...
public(package) fun share(
    ...
    assert!(
        upgrade_cap.package().to_address().to_ascii_string() ==
            type_name::get_with_original_ids<State>().get_address(),
...
#[test_only]
public(package) fun destroy(self: State) {
    let State {
        id,
        trusted_signers: _,
        upgrade_cap,
