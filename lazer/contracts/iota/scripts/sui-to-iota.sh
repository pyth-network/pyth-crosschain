...
# of SUI but is not 100% compatible with SUI. You'll need to run this script when
# reusing Sui contract code and updating the vendored libs.
...
if sed --version >/dev/null 2>&1; then
    ...
    if ! command -v gsed >/dev/null 2>&1; then
        echo "Error: GNU sed (gsed) is required for macOS/BSD. Install gnu-sed via Homebrew."
        exit 1
    fi
    SED_CMD="gsed"
fi
