...
#### `buildAddSignerTx(txBuilder, ...)`

Mints a signer NFT and creates the signing policy UTxO. Only callable by the
contract owner:
...
buildAddSignerTx(
    ...
    signerPubkey,        // Ed25519 pubkey of the trusted signer (hex)
    validFrom?,          // optional: signer valid-from (slot)
    validTo?,            // optional: signer valid-to (slot)
)
```

If `validFrom`/`validTo` are omitted, the signer is valid for all time
(NegInf to PosInf).

#### `buildUpdateSignersTx(txBuilder, ...)`

Updates the signer set by spending the NFT UTxO and recreating it with a new
datum. The NFT is not burned/reminted — it moves to a new output. Only callable
...
#### `buildRemoveSignerTx(txBuilder, ...)`

Burns a signer NFT and spends the signing policy UTxO. Only callable by the
contract owner:
...
buildRemoveSignerTx(
    ...
    signerScriptCbor,
    signerPolicyId,
    ownerPkh,        // owner's pubkey hash (hex)
    signerUtxo,      // the UTxO at the script address to spend
)
```
...
This section walks through the full lifecycle, from initial deployment to
ongoing price verification.
...
const ownerAddr = wallet.getChangeAddress()
...
### 2. Register the Staking Credential (One-Time)
...
### 3. Add a Trusted Signer

```typescript
const PYTH_SIGNER_PUBKEY = '74313a6525edf999...' // from Pyth
...
### 4. Verify Prices in Your dApp Transactions

```typescript
// Get the signer UTxO (query the signer script address for UTxOs with the signer NFT)
const utxos = await provider.fetchAddressUTxOs(signerAddress)
const signerUtxo = utxos.find(u =>
    u.output.amount.some(a => a.unit === signerPolicyId + '7369676e6572')
)

// Get signed price data from Pyth Lazer
const signedPrice: Uint8Array = /* from Pyth Lazer WebSocket */

// Build transaction with price verification
const txBuilder = new MeshTxBuilder({ fetcher: provider, submitter: provider })
...
buildVerifyPriceTx(
    ...
    priceScript.scriptCbor,
    priceRewardAddress,
    { txHash: signerUtxo.input.txHash, outputIndex: signerUtxo.input.outputIndex },
    [signedPrice],
    0,     // validFromSlot
    1000,  // validToSlot
    myChangeAddress,
...
// Add your own dApp logic to the same transaction
// txBuilder.txIn(...).txOut(...)
...
### 5. (Optional) Update the Signer Set
...
### 6. (Optional) Remove a Signer

```typescript
const txBuilder = new MeshTxBuilder({ fetcher: provider, submitter: provider })
buildRemoveSignerTx(txBuilder, signerScript.scriptCbor, signerPolicyId, ownerPkh, signerUtxo)
