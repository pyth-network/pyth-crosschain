...
pub fn verify_update(env: &Env, data: &Bytes) -> Result<Bytes, ContractError> {
    ...
    for i in 0..64 {
        sig_bytes[i] = data.get(4 + i as u32).unwrap();
    }
    let signature = BytesN::from_array(env, &sig_bytes);

    // Parse recovery_id (1 byte at offset 68)
    let recovery_id = data.get(68).unwrap() as u32;
...
fn read_le_u16(data: &Bytes, offset: u32) -> u16 {
    (data.get(offset).unwrap() as u16) | ((data.get(offset + 1).unwrap() as u16) << 8)
}
