//! Vendored verbatim from wormhole-foundation/wormhole `near/contracts/wormhole/src/state.rs`.
//! The VAA wire format parsed here is unchanged from upstream; only the trusted signer set
//! initialized in `lib.rs` differs (Pyth Pro routers instead of the Wormhole guardians).
