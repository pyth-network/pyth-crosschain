...
impl accounts::UpdatePriceFeed {
    pub fn populate(payer: Pubkey, encoded_vaa: Pubkey, feed_id: FeedId, treasury_id: u8) -> Self {
        accounts::UpdatePriceFeed {
            payer,
            encoded_vaa,
            config: get_config_address(),
            treasury: get_treasury_address(treasury_id),
            price_feed_account: get_price_feed_address(feed_id),
...
impl instruction::UpdatePriceFeed {
    pub fn populate(
        ...
        let update_price_feed_accounts =
            accounts::UpdatePriceFeed::populate(payer, encoded_vaa, feed_id, treasury_id)
