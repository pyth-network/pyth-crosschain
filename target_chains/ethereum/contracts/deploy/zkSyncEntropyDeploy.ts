...
import { EvmWormholeContract } from "@pythnetwork/contract-manager";
...
export default async function (hre: HardhatRuntimeEnvironment) {
  ...
  // Check if the wormhole receiver contract is already deployed.
  const wormholeReceiverAddress = process.env.WORMHOLE_RECEIVER_ADDRESS;
  let wormholeReceiverContract = new EvmWormholeContract(
    wormholeReceiverChainId,
    wormholeReceiverAddress
  );

  console.log("wormholeReceiverContract: ", wormholeReceiverContract);


  if (wormholeReceiverContract.address) {
    console.log(
      "WormholeReceiver already deployed at: ",
      wormholeReceiverAddress
    );
  } else {
    console.log("WormholeReceiver not deployed yet, deploying...");
    ...
    console.log("Syncing mainnet guardian sets...");
    await (wormholeReceiverContract as EvmWormholeContract).syncMainnetGuardianSets(wallet.privateKey);
    console.log("✅ Synced mainnet guardian sets");
  }
