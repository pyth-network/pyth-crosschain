...
6. Deploy the new contract or changes using the [`deploy.sh`](./deploy.sh) script. If you have madee changes 
   to [`chains.ts`](../../../governance/xc_governance_sdk_js/src/chains.ts), please make sure to 
   run `npx lerna run build --scope="@pythnetwork/pyth-evm-contract" --include-dependencies` in the root directory before running the deployment script.
