...
pub const CALL_RETDATA_DECODING_ERROR_MESSAGE: &[u8] =
    b"error decoding retdata";
...
pub fn delegate_call_helper<C: SolCall>(
    ...
    let res = unsafe {
        delegate_call(storage, address, &calldata).map_err(map_call_error)?
    };
...
pub fn static_call_helper<C: SolCall>(
    ...
    let calldata = C::new(args).abi_encode();
    let res =
        static_call(storage, address, &calldata).map_err(map_call_error)?;
    C::abi_decode_returns(&res, false /* validate */)
        .map_err(|_| CALL_RETDATA_DECODING_ERROR_MESSAGE.to_vec())
}
