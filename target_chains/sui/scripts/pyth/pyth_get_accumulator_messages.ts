/// Fetch price feed VAAs of interest from the Pyth
/// price feed service.
import { PriceServiceConnection } from "@pythnetwork/price-service-client";

import axios from "axios";
async function main() {
  // Fetch all price IDs
  //let {data} = await axios.post("https://hermes-beta.pyth.network/api/latest_vaas", {ids:["0x2a01deaec9e51a579277b34b122399984d0bbf57e2458a7e42fecd2829867a0d"]})
  let {data} = await axios.get("https://hermes-beta.pyth.network/api/latest_vaas?ids[]=0xf9c0172ba10dfa4d19088d94f5bf61d3b54d5bd7483a322a982e1373ee8ea31b")
  console.log(data);
}
main();

// Step 1: batch price attestation VAA
const vaa =
  "";

// Step 2: sui shared object id
const object_id =
  "0x9a6d328a6ebd906ed842e6e874e14bea69efa1c421fbfe7f689c33412a43c41c";
