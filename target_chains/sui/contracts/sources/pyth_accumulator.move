...
const EEinvalidUpdateData: u64 = 245;
const EInvalidProof: u64 = 345;
const EInvalidWormholeMessage: u64 = 454;
const EInvalidAccumulatorPayload: u64 = 554;
const EInvalidAccumulatorHeader: u64 = 657;
...
// vaa_payload.
public(package) fun parse_and_verify_accumulator_message(
    cursor: &mut Cursor<u8>,
    vaa_payload: vector<u8>,
    clock: &Clock,
): vector<PriceInfo> {
    let header = deserialize::deserialize_u32(cursor);

    if ((header as u64) != PYTHNET_ACCUMULATOR_UPDATE_MAGIC) {
        abort EInvalidAccumulatorHeader
    };

    let major = deserialize::deserialize_u8(cursor);
    assert!(major == MAJOR_VERSION, EInvalidAccumulatorPayload);

    let minor = deserialize::deserialize_u8(cursor);
    assert!(minor >= MINIMUM_SUPPORTED_MINOR_VERSION, EInvalidAccumulatorPayload);
    ...
    assert!(proof_type == 0, EInvalidAccumulatorPayload);
...
fun parse_accumulator_merkle_root_from_vaa_payload(message: vector<u8>): Bytes20 {
    ...
    assert!(
        payload_type == ACCUMULATOR_UPDATE_WORMHOLE_VERIFICATION_MAGIC,
        EInvalidWormholeMessage,
    );
    let wh_message_payload_type = deserialize::deserialize_u8(&mut msg_payload_cursor);
    assert!(wh_message_payload_type == 0, EInvalidWormholeMessage); // Merkle variant
...
fun parse_price_feed_message(message_cur: &mut Cursor<u8>, clock: &Clock): PriceInfo {
    ...
    assert!(message_type == PRICE_FEED_MESSAGE_TYPE, EEinvalidUpdateData);
    let price_identifier = price_identifier::from_byte_vec(
        deserialize::deserialize_vector(message_cur, 32),
    );
...
fun parse_and_verify_accumulator_updates(
    ...
    while (update_size > 0) {
        ...
        assert!(merkle_tree::is_proof_valid(cursor, merkle_root, message), EInvalidProof);
