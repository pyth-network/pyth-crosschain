...
const EPriceInfoRegistryAlreadyExists: u64 = 0;
const EPriceIdentifierAlreadyRegistered: u64 = 1;
const EPriceIdentifierNotRegistered: u64 = 2;
...
public(package) fun new_price_info_registry(parent_id: &mut UID, ctx: &mut TxContext) {
    assert!(!dynamic_object_field::exists_(parent_id, KEY), EPriceInfoRegistryAlreadyExists);
...
public(package) fun add(parent_id: &mut UID, price_identifier: PriceIdentifier, id: ID) {
    assert!(!contains(parent_id, price_identifier), EPriceIdentifierAlreadyRegistered);
    table::add(
...
public fun get_id_bytes(parent_id: &UID, price_identifier: PriceIdentifier): vector<u8> {
    assert!(contains(parent_id, price_identifier), EPriceIdentifierNotRegistered);
    object::id_to_bytes(
