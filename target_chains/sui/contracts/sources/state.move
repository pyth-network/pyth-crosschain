...
public(package) fun set_fee_recipient(_: &LatestOnly, self: &mut State, addr: address) {
...
public(package) fun borrow_mut_consumed_vaas(_: &LatestOnly, self: &mut State): &mut ConsumedVAAs {
...
public(package) fun borrow_mut_consumed_vaas_unchecked(self: &mut State): &mut ConsumedVAAs {
...
public(package) fun set_data_sources(
    _: &LatestOnly,
    s: &mut State,
    mut new_sources: vector<DataSource>,
) {
...
public(package) fun register_price_info_object(
    _: &LatestOnly,
    s: &mut State,
...
public(package) fun set_last_executed_governance_sequence(
    _: &LatestOnly,
    s: &mut State,
    sequence: u64,
) {
...
public(package) fun set_stale_price_threshold_secs(
    _: &LatestOnly,
    s: &mut State,
    threshold_secs: u64,
) {
...
public(package) fun authorize_upgrade(self: &mut State, package_digest: Bytes32): UpgradeTicket {
...
/// a stale build, the transaction will revert with `PackageUpgradeError`,
/// specifically `PackageIDDoesNotMatch`.
public(package) fun commit_upgrade(self: &mut State, receipt: UpgradeReceipt): (ID, ID) {
    let cap = &mut self.upgrade_cap;
...
/// the upgrade.
public(package) fun assert_authorized_digest(_: &LatestOnly, self: &State, digest: Bytes32) {
    let authorized = package_utils::authorized_digest(&self.id);
    assert!(digest == authorized, EInvalidBuildDigest);
}
...
#[test_only]
public fun register_price_info_object_for_test(
    self: &mut State,
    price_identifier: PriceIdentifier,
    id: ID,
) {
...
#[deprecated(note = b"Current Digest is deprecated")]
public struct CurrentDigest has copy, drop, store {}
