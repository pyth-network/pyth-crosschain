...
// The `@pythnetwork/pyth-lazer-solana-sdk` package exports the `PythLazerSolanaContract`
// type but, as of 0.2.0, its published build ships no runtime IDL value
// (`PYTH_LAZER_SOLANA_CONTRACT_IDL` resolves to `undefined`), so the canonical IDL
// JSON is vendored here. Switch back to the SDK's runtime export once it ships one.
import idlJson from "./idl/pyth-lazer-solana-contract.json";
...
const PYTH_LAZER_SOLANA_CONTRACT_IDL =
  idlJson as unknown as PythLazerSolanaContract;
