...
async function main() {
  ...
  for (const contract of Object.values(DefaultStore.entropy_contracts)) {
    if (selectedChains.includes(contract.chain)) {
      ...
      try {
        // Sei EVM (both mainnet and testnet) has unreliable gas estimation, use manual gas limit to bypass estimation
        // HyperEVM testnet also needs manual gas due to big block requirements
        const needsManualGas = 
          contract.chain.getId() === "sei_evm_mainnet" || 
          contract.chain.getId() === "sei_evm_testnet" ||
          contract.chain.getId() === "hyperevm_testnet";
        const gasMultiplier = needsManualGas ? 1 : 2;
        // HyperEVM testnet: use 25M gas for big blocks (30M limit), Sei: 10M gas
        const manualGasLimit = needsManualGas 
          ? (contract.chain.getId() === "hyperevm_testnet" ? 25000000 : 10000000)
          : undefined;
        
        if (needsManualGas) {
          console.log(`Using manual gas limit of ${manualGasLimit} for ${contract.chain.getId()}`);
