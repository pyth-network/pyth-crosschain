...
const CACHE_FILE = ".cache-upgrade-evm-entropy-contract";
const runIfNotCached = makeCacheFunction(CACHE_FILE);

const parser = yargs(hideBin(process.argv))
  .usage(
    "Deploys a new EntropyUpgradable contract to a set of chains and creates a governance proposal for it.\n" +
...
async function main() {
  ...
  for (const contract of Object.values(DefaultStore.entropy_contracts)) {
    if (selectedChains.includes(contract.chain)) {
      ...
      const payload = await contract.generateUpgradeEntropyContractPayload(
        address
      );
