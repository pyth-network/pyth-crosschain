      ...
      if (instruction.governanceAction instanceof EvmExecute) {
        ...
        for (const chain of Object.values(DefaultStore.chains)) {
          if (
            ...
            const args = "0x" + calldataHex.slice(8);

            if (selector === selectorOf("upgradeTo(address)")) {
              const newImplementationAddress = web3.eth.abi.decodeParameter(
                "address",
                args,
              ...
              // this should be the same keccak256 of the deployedCode property generated by truffle
              console.log(
                `${chain.getId()}  call upgradeTo(${newImplementationAddress}) on ${contract.getType()} at address:${callAddress} from executor:${executorAddress}.`,
              );
              console.log(
                `${chain.getId()}    new implementation address:${newImplementationAddress} has code digest:${newImplementationCode}`,
              );
            } else if (selector === selectorOf("setPythFee(uint128)")) {
              const newFee = BigInt(
              ...
              try {
                const currentFeeHex = await chain.getWeb3().eth.call({
                  data: web3.eth.abi.encodeFunctionSignature("getPythFee()"),
                  to: callAddress,
                });
              ...
              console.log(
                `${chain.getId()}  call setPythFee(${newFee}) on ${contract.getType()} at address:${callAddress} from executor:${executorAddress}.`,
              );
              console.log(
                `${chain.getId()}    current on-chain fee: ${currentFee} wei -> proposed: ${newFee} wei`,
              );
            } else if (selector === selectorOf("withdrawFee(address,uint128)")) {
              const decoded = web3.eth.abi.decodeParameters(
                ["address", "uint128"],
                args,
              );
              const targetAddress = decoded[0] as string;
              const amount = BigInt(decoded[1] as string);
              console.log(
                `${chain.getId()}  call withdrawFee(${targetAddress}, ${amount}) on ${contract.getType()} at address:${callAddress} from executor:${executorAddress}.`,
              );
              const targetCode = await chain
                .getWeb3()
                .eth.getCode(targetAddress);
              if (targetCode === "0x" || targetCode === "") {
                console.log(
                  `${chain.getId()}    WARNING: withdraw target ${targetAddress} has NO code on this chain (expected a deployed multisig)`,
                );
              } else {
                console.log(
                  `${chain.getId()}    withdraw target ${targetAddress} is a deployed contract (code digest:${Web3.utils.keccak256(targetCode)})`,
                );
              }
              if (contract instanceof EvmEntropyContract) {
                ...
                if (amount > accrued) {
                  ...
                  console.log(
                    `${chain.getId()}    amount ${amount} <= current accrued fees ${accrued}`,
                  );
                }
              }
            } else {
              ...
              console.log(
                `${chain.getId()}    raw calldata: 0x${calldataHex}`,
              );
            }
          }
        }
