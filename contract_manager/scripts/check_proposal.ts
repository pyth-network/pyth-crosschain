...
async function main() {
  ...
  for (const [index, instruction] of parsedInstructions.entries()) {
    // Without this, a single unreachable RPC aborts the run and leaves every
    // instruction after it unchecked.
    try {
      await checkInstruction(instruction, cluster, argv);
    } catch (error) {
      console.error(`Failed to check instruction ${index + 1}:`, error);
    }
  }
}
...
async function checkInstruction(
  ...
  cluster: PythCluster,
  argv: Awaited<typeof parser.argv>,
) {
  if (instruction instanceof WormholeMultisigInstruction) {
    if (instruction.governanceAction instanceof EvmSetWormholeAddress) {
      ...
      for (const chain of Object.values(DefaultStore.chains)) {
        if (
          chain instanceof EvmChain &&
          chain.wormholeChainName === instruction.governanceAction.targetChainId
        ) {
    ...
    if (instruction.governanceAction instanceof SetDataSources) {
      ...
      for (const source of dataSources) {
        console.log(
          `  new data source: chain ${source.emitterChain} emitter 0x${source.emitterAddress}`,
        );
      }
      for (const chain of evmChainsFor(targetChainId, cluster)) {
        for (const contract of pythContractsOn(chain)) {
          for (const source of await contract.getDataSources()) {
            console.log(
              `${chain.getId()}  Address:${contract.address} current data source: chain ${source.emitterChain} emitter 0x${source.emitterAddress}`,
            );
          }
        }
      }
    ...
    if (instruction.governanceAction instanceof EvmExecute) {
      ...
      for (const chain of Object.values(DefaultStore.chains)) {
        if (
          ...
          const executorAddress = instruction.governanceAction.executorAddress;
