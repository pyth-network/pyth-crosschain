...
test("GovernancePayload ser/de", (done) => {
  ...
  buffer = Buffer.alloc(1000);
  span = encodeExecutePostedVaa(expectedExecuteVaaArgs, buffer);
  expect(
    buffer
      .subarray(0, span)
      .equals(Buffer.from([80, 84, 71, 77, 0, 0, 0, 26, 0, 0, 0, 0]))
  ).toBeTruthy();
  let executePostedVaaArgs = decodeExecutePostedVaa(buffer.subarray(0, span));
  ...
  executePostedVaaArgs = decodeExecutePostedVaa(buffer.subarray(0, span));
  expect(executePostedVaaArgs?.header.targetChainId).toBe("pythnet");
  expect(executePostedVaaArgs?.header.action).toBe("ExecutePostedVaa");
