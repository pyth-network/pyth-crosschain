...
import {CosmwasmExecutor, PythWrapperExecutor, PythWrapperQuerier} from "@pythnetwork/cosmwasm-deploy-tools";
import {
...
export class CosmWasmContract {
    ...
    static from(path: string): CosmWasmContract {
        ...
        return new CosmWasmContract(Chains[parsed.chain] as CosmWasmChain, parsed.address);
    }

    //TODO : make deploymentType enum stable  | edge
    static getDeploymentConfig(chain: CosmWasmChain, deploymentType: string, wormholeContract: string): CosmWasmContract.DeploymentConfig {
    ...
    static async deploy(chain: CosmWasmChain, codeId: number, config: CosmWasmContract.DeploymentConfig, mnemonic: string): Promise<any> {
        let executor = new CosmwasmExecutor(chain.executorEndpoint, mnemonic, chain.prefix, chain.gasPrice + chain.feeDenom);
    ...
    to(path: string): void {
        writeFileSync(`${path}/${this.getId()}.${CosmWasmContract.type}.json`, JSON.stringify({
            chain: this.chain.id,
    ...
    async getConfig() {
        ...
        let allStates = (await chainQuerier.getAllContractState({contractAddr: this.address})) as any;
        let config = {
    ...
    async getDeploymentType(): Promise<string> {
        ...
        console.log(config.config_v1.data_sources, stableConfig.data_sources, edgeConfig.data_sources)
        if (this.equalDataSources(config.config_v1.data_sources, stableConfig.data_sources))
            return "stable";
        else if (this.equalDataSources(config.config_v1.data_sources, edgeConfig.data_sources))
            return "edge";
        else
            return "unknown";
    }
    ...
    async updatePriceFeed(mnemonic: string) {
        ...
        console.log(fund)
        let executor = new CosmwasmExecutor(this.chain.executorEndpoint, mnemonic, this.chain.prefix, this.chain.gasPrice + this.chain.feeDenom);
    ...
    async getValidTimePeriod(): Promise<any> {
        ...
        let result = await client.queryContractSmart(this.address, 'get_valid_time_period');
        return result;
    }
}