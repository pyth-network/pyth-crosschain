...
export class Listener {
  ...
  start() {
    ...
    const batchPriceFeedUpdateEmitter = this.pythContract.events.BatchPriceFeedUpdate(undefined, async (_error: Error, event: EventData) => {
      ...
      const waitForPricesCondition = () => this.batchPrices.has(batchHash) && this.batchPrices.get(batchHash)!.length == batchSize;
      const result = await waitForCondition(waitForPricesCondition, 5000);
      if (result === false) {
        console.error(`The price list for ${batchHash} didn't complete. Reason: Timeout`);
        return;
      }
    ...
    const UpdatePriceFeedsEmitter = this.pythContract.events.UpdatePriceFeeds(undefined, async (_error: Error, event: EventData) => {
      ...
      const waitForBatchesCondition = () => this.txBatches.has(txHash) && this.txBatches.get(txHash)!.length == batchCount;
      const waitForBatchesResult = await waitForCondition(waitForBatchesCondition, 5000);
      if (waitForBatchesResult === false) {
        console.error(`Couldn't fetch all batches in transaction ${txHash}. Reason: Timeout`);
        return;
      }
      ...
      const waitForTimestampCondition = () => this.blockToTimestamp.has(event.blockNumber);
      const waitForTimestampResult = await waitForCondition(waitForTimestampCondition, 5000);
      if (waitForTimestampResult === false) {
        console.error(`Couldn't fetch timestamp of transaction ${txHash}. Reason: Timeout`);
        return;
      }
