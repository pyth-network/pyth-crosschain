  ...
  if (instruction instanceof WormholeMultisigInstruction) {
    ...
    if (governanceAction instanceof ExecutePostedVaa) {
      return governanceAction.instructions.map((innerInstruction) =>
        getTransactionSummary(parser, innerInstruction)
      )
    ...
    return [{ name: instruction.name } as const]
  }
}
...
const getTransactionSummary = (
  ...
  switch (parsedInstruction.name) {
    case 'addPublisher':
      return {
        name: 'addPublisher',
        priceAccount:
          parsedInstruction.accounts.named['priceAccount'].pubkey.toBase58(),
        pub: (parsedInstruction.args['pub'] as PublicKey).toBase58(),
      } as const
    case 'delPublisher':
      return {
        name: 'delPublisher',
