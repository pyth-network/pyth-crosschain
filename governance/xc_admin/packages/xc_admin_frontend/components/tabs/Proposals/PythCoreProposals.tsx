...
interface PythCoreProposalsProps {
  proposalPubkey?: string
  onBackToProposals?: () => void
}

const PythCoreProposals = ({
  proposalPubkey,
  onBackToProposals,
}: PythCoreProposalsProps) => {
  ...
  useEffect(() => {
    if (proposalPubkey) {
      const currProposal = multisigProposals.find(
        (proposal) => proposal.publicKey.toBase58() === proposalPubkey
      )
  ...
  if (proposalPubkey) {
    if (!isMultisigLoading && currentProposal !== undefined) {
      return (
        <>
          <div
        ...
        {isMultisigLoading ? (
          ...
          <>
            ...
            {filteredProposals.length > 0 ? (
              ...
              <div className="mt-4">
                No proposals found. If you&apos;re a member of the multisig, you
...
export default PythCoreProposals
