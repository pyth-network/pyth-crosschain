...
<div>If you choose the price feed account integration (see below), you
can use an account address check to validate the price feed ID.</div>
...
<InfoBox variant="info">
  <div>Posting and verifying price updates currently requires multiple transactions
  ...
  partially verified price updates.</div>
</InfoBox>
