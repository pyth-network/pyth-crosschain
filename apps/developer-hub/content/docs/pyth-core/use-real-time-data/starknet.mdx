...
<InfoBox variant="info">
  <div>Unlike Ethereum, there is no native token on Starknet. You cannot pass tokens
  ...
  currently set to the minimum possible value (1e-18 STRK, 1 WEI).</div>
</InfoBox>
...
```
<InfoBox variant="info">
<div>Price updates must be converted to `ByteBuffer` before being passed on to the
  Pyth contract on Starknet. Use the `ByteBuffer` type from
  `@pythnetwork/pyth-starknet-js` package as shown above.</div>
</InfoBox>
