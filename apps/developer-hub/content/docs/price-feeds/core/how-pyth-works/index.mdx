...
Pyth's oracle program runs on [Pythnet](./pythnet).
The prices constructed on Pythnet are transferred [cross-chain](./cross-chain) to reach consumers on these blockchains.

The critical component of the system is the [oracle program](./oracle-program) that combines the data from each individual publisher.
...
The first price update in a slot additionally triggers [price aggregation](./price-aggregation), which combines the price data from the previous slot into a single aggregate price and confidence interval.
