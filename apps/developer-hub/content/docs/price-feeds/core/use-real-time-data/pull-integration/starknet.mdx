---
title: How to Use Real-Time Data in Starknet Contracts
description: Consume Pyth Network prices in Starknet applications
slug: /price-feeds/core/use-real-time-data/pull-integration/starknet
---
...
<Tabs items={["npm", "yarn"]}>
  <Tab value="npm">
    ```bash copy npm install --save @pythnetwork/pyth-starknet-js ```
  </Tab>
  <Tab value="yarn">
    ```bash copy yarn add @pythnetwork/pyth-starknet-js ```
  </Tab>
</Tabs>
...
1. Call `pyth.get_price_no_older_than` to read the price, providing the [price feed ID](/price-feeds/core/price-feeds) you wish to read.
...
<Callout type="info" title="Convert to ByteBuffer">
