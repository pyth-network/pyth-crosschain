...
1. **Aggregate & Sign**: Each tick, the five Pyth Pro routers independently
   compute the price aggregates for all price feeds, compress them into a Merkle tree using Pythnet's leaf
...
4. **On-Chain Verification**: The upgraded contracts verify that the router
   signatures meet the quorum and validate the requested price update against the Merkle
   root.
