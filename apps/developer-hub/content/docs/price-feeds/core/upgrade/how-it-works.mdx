...
1. **Aggregate & Sign**: Each tick, the five Pyth Pro routers independently
   compute the price aggregate, build a Merkle tree using Pythnet's leaf
   format, and sign the resulting Merkle root.
...
4. **On-Chain Verification**: The upgraded contracts verify that the router
   signatures meet the quorum and validate the requested price against the Merkle
   root.
...
| **Hermes Compatibility** | Standard Hermes Endpoint | Upgraded Hermes Endpoint (Byte-Compatible API) |
