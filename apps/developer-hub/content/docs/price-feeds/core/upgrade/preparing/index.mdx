...
<Accordions type="single">
  ...
  <Accordion title="My code uses the Hermes REST/SSE API directly — what changes for me?">
    ...
    - **Automatic upgrade:** keep calling `hermes.pyth.network` without the header today. At the cutover on July 31, push a deploy that adds the `Authorization: Bearer $PYTH_API_KEY` header on the same URL. See [Waiting for the Automatic Upgrade](/price-feeds/core/upgrade/preparing#waiting-for-the-automatic-upgrade) for the gap-handling pattern.
