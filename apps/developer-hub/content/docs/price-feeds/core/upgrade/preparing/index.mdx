...
<Callout type="warn">
  **Deploy Step 2 and Step 3 in the same release.**
  ...
  If you cannot deploy both together, use the forward-compatible strategy in the Automatic Upgrade section.
</Callout>
...
During that brief gap, the price updates serverd by `hermes.pyth.network` cannot verify against your upgraded contract, while those served by `pyth.dourolabs.app/hermes` can.
...
<Accordions type="single">
  ...
  <Accordion title="My code uses the Hermes REST/SSE API directly — what changes for me?">
    ...
    - **Upgrade early (recommended):** switch your base URL to `pyth.dourolabs.app/hermes` and start sending the API key to your requests. The data is identical; only auth changes. See the [upgrade guide](/price-feeds/core/upgrade/preparing) for the full path.
