...
This is the recommended path, as it makes the **July 31 cutover a non-event for your integration**. You can switch to the new endpoint and swap addresses in the same deploy.
...
Switch your Hermes base URL from [`hermes.pyth.network`](https://hermes.pyth.network/docs/) to [`pyth.dourolabs.app/hermes`](https://pyth.dourolabs.app/docs/?urls.primaryName=Hermes+API) and add your API key that you got in Step 1. The two URLs serve the same data today. The upgraded endpoint requires authentication.
...
The upgraded endpoint is a drop-in replacement. See the new [Hermes API reference](https://pyth.dourolabs.app/docs/?urls.primaryName=Hermes+API#/) for the full surface. Note that this is the new endpoint, not the old one.

<Callout type="warn">
  **Pair this with Step 2 in the same deploy.** The new endpoint serves router-signed payloads that only the upgraded Pyth Core Contract verifies.
</Callout>
...
## If you're waiting for the automatic upgrade
...
During that brief gap, `hermes.pyth.network` cannot verify against your upgraded contract, while `pyth.dourolabs.app/hermes` can.

To stay live without timing the switch yourself, fetch from both URLs every request and submit whichever payload verifies:
...
<Accordions type="single">
  <Accordion title="What happens if I do nothing?">
    **Your Hermes calls will start getting rejected at the cutover on July 31, 2026** — `hermes.pyth.network` will require an API key from that point on. To stay live, two things you need to do:

    1. Have a Pyth API Key (Step 1) by July 31.
    2. Push a code change adding `Authorization: Bearer $PYTH_API_KEY` header to your Hermes requests, around the cutover.

    Your on-chain contract is handled for you — the Pyth DAO upgrades it on July 31, no action needed there.

    Most users avoid this last-minute risk by upgrading early (see Steps 2 and 3) and switching to the upgraded Hermes endpoint, which already accepts the API key today.
  </Accordion>
  <Accordion title="Will my existing code break?">
    ...
    Hermes code breaks at the cutover *if* your client is still pointed at `hermes.pyth.network` without an API key. Either upgrade early (Step 2) or plan a deploy around July 31 to add the header.
  </Accordion>
  <Accordion title="When exactly does Hermes redirect?">
    On July 31, 2026. Exact switch timing is announced closer to the date. From that point, `hermes.pyth.network` serves the upgraded data and requires API key authentication — unauthenticated requests will be rejected.
  </Accordion>
  ...
  <Accordion title="My code uses the Hermes REST/SSE API directly — what changes for me?">
    Routes and response shapes are unchanged — the upgraded endpoint is a drop-in replacement. The differences are *which URL* you call and *whether you send an API key*:

    - **Upgrade early (recommended):** switch your base URL to `pyth.dourolabs.app/hermes` and add `Authorization: Bearer $PYTH_API_KEY` header today. The data is identical; only auth changes.
    - **Wait:** stay on `hermes.pyth.network` today (unauthenticated). On July 31, push a deploy that adds the API key header on the same URL.
