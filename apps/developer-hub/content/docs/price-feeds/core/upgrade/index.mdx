...
<Accordions type="single">
  <Accordion title="What happens if I do nothing?">
    **Your Hermes calls will start getting rejected at the cutover on July 31, 2026** — `hermes.pyth.network` will require an API key from that point on. To stay live, please follow the steps mentioned [above](#if-youre-waiting-for-the-automatic-upgrade).
  </Accordion>
  <Accordion title="Will my existing code break?">
    ...
    Hermes code breaks at the cutover *if* your client is still pointed at `hermes.pyth.network` without an API key. Please follow the steps mentioned [above](#if-youre-waiting-for-the-automatic-upgrade).
  </Accordion>
  ...
  <Accordion title="My code uses the Hermes REST/SSE API directly — what changes for me?">
    ...
    - **Upgrade early (recommended):** switch your base URL to `pyth.dourolabs.app/hermes` and add `Authorization: Bearer $PYTH_API_KEY` header today. The data is identical; only auth changes. See the [upgrade guide](/price-feeds/core/upgrade) for the full path.
