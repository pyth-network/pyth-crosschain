...
- [TradingView integration](./create-tradingview-charts) for visualization. The Benchmarks `/v1/shims/tradingview/*` endpoints and `/v1/price_differences` are retired at the upgrade — charting moves to the [Pyth Pro History API](/price-feeds/pro/api/history).
