...
- Risk parameter configuration (margin tables, OI caps, fees)
- Trading halts
...
<Steps>
  ...
  <Step>**Publishes** a single `setOracle` call containing all asset prices</Step>
</Steps>
...
- A price is considered **stale** if older than the configured threshold (default: 5 seconds).
- If **all sources fail** for a symbol, that symbol is excluded from the update. No fabricated prices are ever published.
...
<Callout type="default" title="Enforced by Hyperliquid">
Hyperliquid applies additional safeguards to all oracle updates. These are enforced regardless of what the pusher publishes:

- **Daily price cap**: Prices clamped at 10× start-of-day price
- **Mark change limit**: Each update clamped to ±1% from previous mark
- **OI protection**: Updates rejected if implied OI would exceed 10× configured cap
</Callout>
...
Each `setOracle` call includes, per asset, the fields below. If mark prices are not provided, Hyperliquid uses its local mark exclusively. See the [HIP-3 as a Service](/price-feeds/pro/hip-3-service) overview for context.
</Callout>
...
<Cards>
  <Card icon={<Key size={18} />} title="Local Key: Development / Low-value testing">
    Private key stored in environment or configuration file. Suitable for testnet or development environments.
  </Card>
  ...
  <Card icon={<UsersThree size={18} />} title="Multisig: Production / High-value">
...
The pusher's signing key is restricted to price updates only. The following permissions apply:

<Callout type="default" title="Sub-deployer permissions">
The pusher's signing key is configured as a **sub-deployer** with restricted permissions:

**Allowed:** `setOracle` (all price inputs)

**Not Allowed:** Risk parameter changes (OI caps, margin tables, fees), trading halts, market listing/delisting, and any deployer-level operations.

This separation ensures the pusher can only publish prices. It cannot modify market parameters or halt trading, even if the signing key were compromised.
</Callout>
...
<Steps>
  ...
  <Step>All sources for a symbol fail → Symbol excluded (no stale/fabricated prices)</Step>
  <Step>Critical sources unavailable → Alerts triggered, publishing continues for healthy symbols</Step>
</Steps>
...
- **Override protocol safeguards**: Hyperliquid's price clamps and OI limits are enforced regardless of pusher inputs
...
Each data source has its own requirements. The following apply:

- **Pyth Pro / Pythnet**: Requires valid API credentials and network connectivity to Pyth infrastructure
- **SEDA**: Requires network access to configured endpoints
- **Hyperliquid**: Requires WebSocket connectivity to Hyperliquid nodes
...
Set the staleness threshold to **3–5 seconds** for production. This balances freshness against availability and avoids rejecting valid prices during minor network delays while still excluding prices that no longer reflect current market conditions.
</Callout>
