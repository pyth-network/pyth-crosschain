...
The Pyth MCP server gives AI assistants direct, structured access to Pyth market data through the [Model Context Protocol](https://modelcontextprotocol.io/) — discover feeds, fetch real-time and historical prices, and generate chart-ready candles, all within a single conversation.
...
<Callout type="warning">
  `get_latest_price` requires a Pyth Pro access token passed as `access_token`. Get one at [pyth.network/pricing](https://pyth.network/pricing). All other tools work without a token.
</Callout>
