...
The same JWT also authenticates the REST API on `https://pyth-lazer.dourolabs.app`: send it as `Authorization: Bearer <JWT>` wherever the docs show the API key.

That bearer-header flow covers the History and REST APIs. A browser cannot attach an `Authorization` header to a native WebSocket, so keep WebSocket streaming behind your backend — open the authenticated socket there and relay updates to the client.
