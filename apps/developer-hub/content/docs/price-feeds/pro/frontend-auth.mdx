...
| Browser — History API      | Your backend calls `POST /auth/token` with the Pro API key and returns a short-lived JWT to the browser, which sends it as `Authorization: Bearer <JWT>`. |
| Browser — REST / WebSocket | Route the request through your backend; the browser never holds the API key. The JWT does not authenticate these APIs. |
...
4. The browser uses the JWT as a bearer token (`Authorization: Bearer <JWT>`) until it nears expiry, then repeats from step 1.
...
This JWT is scoped to the History API on `https://pyth.dourolabs.app` — the host that mints it. It does not authenticate the REST or WebSocket APIs on `https://pyth-lazer.dourolabs.app`.

To reach those from a browser, keep the request behind your backend: hold the API key there, call the REST or WebSocket API from your server, and relay the result to the client.
