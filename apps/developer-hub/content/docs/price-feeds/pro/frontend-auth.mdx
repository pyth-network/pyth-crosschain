...
The server reads the token from that value, authenticates, and completes the handshake by echoing back the `pyth-lazer-auth` subprotocol — your client accepts it and does nothing with it. The rest of the [subscribe flow](/price-feeds/pro/subscribe-to-prices) is unchanged.
