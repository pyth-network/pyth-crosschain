...
Browser bundles are public: anyone can read the code you ship to the client, so a long-lived `PRO_API_KEY` must never appear in frontend code. Instead, your backend mints a short-lived JWT through `POST /auth/token` and hands it to the browser, which sends it to the History, REST, and WebSocket APIs in place of the API key.
...
4. The browser sends the JWT with each Pyth Pro request in place of the API key, and repeats from step 1 as the token nears expiry.
...
The same JWT works against the REST API on `https://pyth-lazer.dourolabs.app` the same way: send it as `Authorization: Bearer <JWT>` wherever the docs show the API key.

The WebSocket API works too, with one difference: a browser cannot set an `Authorization` header on a WebSocket connection, so pass the JWT as the `ACCESS_TOKEN` query parameter on the connection URL instead.

```
wss://pyth-lazer-0.dourolabs.app/v1/stream?ACCESS_TOKEN=<JWT>
```

The server reads the token from the header or this query parameter, so the rest of the [subscribe flow](/price-feeds/pro/subscribe-to-prices) stays the same. Because the token is part of the URL, keep its lifetime short.
