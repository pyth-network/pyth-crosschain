...
<Callout type="info" title="Expected Behavior During Deployments">
  During routine deployments, one endpoint may temporarily return connection errors while the others remain operational. If you're using the official SDK or Rust client, you may see **INFO-level logs** showing retry attempts for a single endpoint — this is normal and expected behavior.

  - **INFO logs with retries**: One endpoint is temporarily down, others are serving data. No action needed.
  - **ERROR logs (all connections dropped)**: All endpoints are unreachable. Check [status.pyth.network](https://status.pyth.network) for outages.

  As long as you're connected to all three endpoints, your application will continue receiving data seamlessly during single-endpoint maintenance.
</Callout>
...
<Callout type="info" title="Recommended: Include feedUpdateTimestamp">
  For production applications, include `"feedUpdateTimestamp"` in your
...
1. Pyth Lazer provides an [SDK](https://www.npmjs.com/package/@pythnetwork/pyth-lazer-sdk) to seamlessly integrate the websocket API into your application.
...
2. Then create a `PythLazerClient` object using both endpoint URLs and the access token requested from our authorized distribution partners (Pyth Data Distributors) in the first step:
