...
A browser can't set the `Authorization` header on a WebSocket. For frontend apps, mint a short-lived JWT (see [Frontend Authentication](/price-feeds/pro/frontend-auth)) and pass it as the `ACCESS_TOKEN` query parameter on the connection URL instead:
