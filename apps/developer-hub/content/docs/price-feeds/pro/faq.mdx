...
Do not embed your Pro access token in browsers or frontend apps. Route requests through your own backend that injects the `Authorization: Bearer <PRO_API_KEY>` header before forwarding. For a client-safe short-lived-JWT flow, see [Frontend Authentication](/price-feeds/pro/frontend-auth).
