...
Skills are pre-built prompt templates that teach AI assistants specific workflows using the Pyth MCP server. Instead of figuring out which tools to call and how to combine them, you can simply describe what you want in natural language and the skill handles the rest.
...
<Callout type="info">
  Skills require the [Pyth MCP server](/price-feeds/pro/mcp#setup) to be configured in your AI assistant. Some skills also require a [Pyth Pro access token](./acquire-access-token).
</Callout>
