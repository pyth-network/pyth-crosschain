...
- "What's the difference between Pyth Pro and Pyth Core?"
...
> "What's the feed ID for AAPL?"
>
> The assistant searches for AAPL and returns the symbol (`Equity.US.AAPL`), the Pyth Pro (Lazer) numeric ID, the Hermes hex ID (for on-chain use), and the exponent for price conversion.
