...
After Step 1, choose how and when to move to the upgraded infrastructure.
...
| Effort           | 2 more steps (~15 min)                     | A code deploy required around July 31                                |

**Upgrade now (recommended)** — Continue with Steps 2 and 3 below. You move to the upgraded Hermes endpoint and Pyth Classic Contract today, on your own schedule, with zero downtime.
...
<Accordions type="single">
  ...
  <Accordion title="Will my existing code break?">
    On-chain code is fine: the upgrade is fully backward-compatible with the existing Pyth Core interface via the Pyth Classic Contract.
  ...
  <Accordion title="Can I test before upgrading?">
    Yes. The upgraded endpoint (`pyth.dourolabs.app/hermes`) and the Pyth Classic Contract are both available on testnets today. See the chain support section above for testnet contract addresses.
  </Accordion>
