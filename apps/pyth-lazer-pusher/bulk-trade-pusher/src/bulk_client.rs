...
impl BulkClient {
    pub async fn start(
        ...
        let mut client =
            WebsocketDeliveryClient::new(config.endpoints.clone()).with_metrics(metrics);
        client.start_all(runtime);
    ...
    loop {
        tokio::select! {
            Some(tx) = tx_rx.recv() => {
                ...
                let sent = client.send_all_same(&json).await;
                debug!(request_id = id, sent_count = sent, "sent transaction");
                metrics::update_bulk_connections(client.connected_count_sync());
            }
