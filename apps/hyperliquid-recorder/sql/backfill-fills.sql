-- Backfill fills and liquidations for a single day.
-- Parameters:
...
INSERT INTO { database :Identifier }.{ trades_table :Identifier } (
        ...
        WHERE coin LIKE 'xyz:%'
            AND timestamp >= toDateTime64({ day_start :String }, 3, 'UTC')
            AND timestamp < toDateTime64({ day_end :String }, 3, 'UTC')
    )
SELECT s.*
FROM source_rows AS s LEFT ANTI
    JOIN { database :Identifier }.{ trades_table :Identifier } AS t ON s.coin = t.coin
    ...
    AND s.user = t.user
    AND s.side = t.side
    AND t.trade_time >= toDateTime64({ day_start :String }, 3, 'UTC')
    AND t.trade_time < toDateTime64({ day_end :String }, 3, 'UTC');
