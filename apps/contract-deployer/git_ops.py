...
class GitOps(object):
    def __init__(self, github_token: str, repo_name: str, commiter_name: str, commiter_email: str,
                 branch_name: str = "contract-deployer") -> None:
        self.github = Github(auth=Auth.Token(github_token))
        self.github_token = github_token
        self.repo_name = repo_name
        self.branch_name = branch_name
        self.signature = Signature(commiter_name, commiter_email)
        self.auth_callback = pygit2.RemoteCallbacks(pygit2.UserPass('x-access-token', self.github_token))
    ...
    def setup(self) -> None:
        self.checkout_path = tempfile.mkdtemp(prefix="python-crosschain-contract-deployer-")
        self.github_repo = self.github.get_repo(self.repo_name)

        # TODO if the branch doesn't exist, create it
        logger.info(f"Cloning {self.repo_name} into {self.checkout_path} (branch: {self.branch_name})...")
        self.repo = pygit2.clone_repository(self.github_repo.clone_url, self.checkout_path,
                                            checkout_branch=self.branch_name, callbacks=self.auth_callback)

        logger.info(f"Merging main branch into {self.branch_name}...")
        self.repo.merge(f'refs/remotes/origin/{self.branch_name}')
    ...
    def get_diff(self):
        self.repo.index.add_all()
        self.repo.index.write()
        return self.repo.diff(cached=True).patch

    def get_checkout_path(self):
        return self.checkout_path

    def create_pull_request(self, pr_title: str, pr_body: str | None = None):
        return self.github_repo.create_pull(title=pr_title, body=pr_body, head=self.branch_name, base=GIT_BASE_BRANCH)
