...
///
/// Ported from `ondo-recorder`'s `HealthState`, keyed by a bare symbol instead
/// of a `(symbol, chain)` market: this recorder streams a single venue, so
/// per-symbol data freshness is the only liveness signal. The SDK owns
/// reconnection internally, so a symbol that never sends data (e.g. one that
/// isn't actually listed on Binance spot) surfaces as perpetual staleness here
...
#[derive(Default)]
struct HealthInner {
    symbol_last_seen: HashMap<String, f64>,
    clickhouse_ok: bool,
}
...
impl HealthState {
    ...
    /// Record that a fresh `bookTicker` update was received for `symbol`.
    pub fn set_symbol_seen(&self, symbol: &str) {
    ...
    fn to_ready_response(&self) -> ReadyResponse {
        ...
        ReadyResponse {
            ready: inner.clickhouse_ok && stale_symbols.is_empty(),
