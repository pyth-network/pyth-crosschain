...
async fn run() -> Result<()> {
    ...
    tracing::info!(instruments = ?config.instruments, "starting okx-recorder");

    let writer_client = create_client_with_retry(config.clone(), 30).await?;
    let metrics = Arc::new(RecorderMetrics::new()?);
    let health = HealthState::new(config.instruments.clone(), config.ready_stale_seconds);
...
fn init_logging() {
    let filter = tracing_subscriber::EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        tracing_subscriber::EnvFilter::new("info,tungstenite=warn,tokio_tungstenite=warn")
    });
